                  ...
                  <div className="w-[180px] h-[180px] bg-white border border-slate-200 rounded-xl p-2.5 flex items-center justify-center shadow-inner relative">
                    <QrCanvas data={
                      selectedDeviceForQr === 'global'
                        ? `https://shieldparent.vercel.app/?enroll=global&parent=vibeai789%40gmail.com&key=8b2a-fd3c`
                        : (() => {
                            const dev = devices.find(d => d.id === selectedDeviceForQr);
                            return `https://shieldparent.vercel.app/?enroll=device&name=${encodeURIComponent(dev?.name || 'Unknown Device')}&platform=${dev?.platform || 'Android'}&uuid=${dev?.uuid || 'unknown'}&ipAddress=${dev?.ipAddress || '192.168.1.100'}&blockAdult=${dev?.blockAdult}&blockGambling=${dev?.blockGambling}&blockSocial=${dev?.blockSocial}`;
                          })()
                    } />
                  </div>
                ...
                <div className="space-y-1.5 text-left">
                  ...
                  <div className="flex gap-1.5">
                    <input
                      ...
                      value={
                        selectedDeviceForQr === 'global'
                          ? `https://shieldparent.vercel.app/?enroll=global&parent=vibeai789%40gmail.com`
                          : (() => {
                              const dev = devices.find(d => d.id === selectedDeviceForQr);
                              return `https://shieldparent.vercel.app/?enroll=device&name=${encodeURIComponent(dev?.name || 'Unknown Device')}&platform=${dev?.platform || 'Android'}&uuid=${dev?.uuid || 'unknown'}&ipAddress=${dev?.ipAddress || '192.168.1.100'}&blockAdult=${dev?.blockAdult}&blockGambling=${dev?.blockGambling}&blockSocial=${dev?.blockSocial}`;
                            })()
